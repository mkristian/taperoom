#
# taperoom - application to manage audio files and give out download tickets
# Copyright (C) 2013 Christian Meier <m.kristian@web.de>
#
# This file is part of taperoom.
#
# taperoom is free software: you can redistribute it and/or modify
# it under the terms of the GNU Affero General Public License as
# published by the Free Software Foundation, either version 3 of the
# License, or (at your option) any later version.
#
# taperoom is distributed in the hope that it will be useful,
# but WITHOUT ANY WARRANTY; without even the implied warranty of
# MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
# GNU Affero General Public License for more details.
#
# You should have received a copy of the GNU Affero General Public License
# along with taperoom.  If not, see <http://www.gnu.org/licenses/>.
#
class Views::Download::Overview < Views::Layouts::Download

  def initialize(view, assigns, stream)
    title = assigns['message'].nil? ? _("download overview") : "#{assigns['message']}"
    super(view, assigns, stream, title)

    @overview = Views::Orders::OverviewWidget.new(view, assigns, stream)
  end

  def render_body
    fieldset :class => :download do

      legend((@order.items.size > 1 ? _("Downloads") : _("Download")) + " " + _("for") + " #{@order.name}:")

      div  :class => :copyright do
        text _("This material is copyrighted, and all rights are reserved. Please do not redistribute this material.")
        rawtext _("Read more")
      end

      div :class => :message do
        b @message
      end

      @overview.render_to(self)

    end
  end

  def render_header
    div do
      button_to _('logout'), @baseurl + "/", :method => :delete, :class => :button
    end
  end
end